...
| `--ring` | `oklch(0.525 0.09 195)` | #007A7A | foco; "+" del logo sobre claro; hover de botón teal |
...
- **Logo** (`components/brand/Logo.tsx`): tipográfico, Outfit 500 minúsculas, "+" teal. `tone="dark"` sobre navy usa `--primary`; `tone="light"` sobre claro usa `--ring` **a cualquier tamaño** (ver la nota de contraste de arriba: no existe umbral por tamaño). Header 28/32px, footer 36px. Lleva `role="img"` + `aria-label="Pensión+"`. El hero no repite el wordmark.
...
- **Fotografía**: `next/image` siempre, `width`/`height` explícitos, `alt` en español y `sizes` en las de dos columnas. `persona-hero.png` (725×700, `priority`, recorte a 210/280/370px de alto) en el hero; `asesoria-mujer.jpg` (1200×800) en la sección de ahorro de 40-65; `exito-whatsapp.jpg` (900×700, `object-cover`) en el CTA final; `avatar-1/2/3.jpg` (160×160) en testimonios y en `HeroProof`. Todas en `rounded-2xl`, sin borde ni sombra. Cero URLs externas.
  `asesoria-datos.jpg` queda **sin uso**: ilustraba la sección "¿Tienes 30 años?" con adultos mayores, que contradice a quien le habla esa sección. Se conserva el archivo como reserva; esa sección va sin foto.
