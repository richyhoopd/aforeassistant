...
- **TestimonialsCarousel** (`components/landing/TestimonialsCarousel.tsx`): reseñas en pista horizontal con scroll nativo y `snap-mandatory`; cards `w-[85%]` en móvil / 420px en desktop, espaciador final para que la última card alcance su punto de ajuste; flechas de 44px y puntos; `prefers-reduced-motion` desplaza sin animación. Vive en "Por qué Pensión+" (navy, patrón de billetes).
