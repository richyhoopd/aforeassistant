...
def test_procesar_item_preview_usa_caption_del_canal_correcto(monkeypatch, tmp_path):
    item = {"tema": "requisitos", "formato": "mito", "plantilla": "tarjeta",
            "channels": ["fb_page", "ig"], "scheduled_at": datetime(2026, 8, 3, tzinfo=timezone.utc), "source": "fb_page"}
    copy = {"titulo": "titulo generico", "bullets": ["a"],
            "captions": {"fb": "caption real de facebook", "ig": "y", "grupo": "", "tiktok": ""}, "laminas": []}
    png = tmp_path / "i.png"; png.write_bytes(b"p")
    monkeypatch.setattr(g, "generate_copy", lambda it, feedback=None: copy)
    monkeypatch.setattr(g, "render_item", lambda it, c, d: [png])
    enviados = {}
    monkeypatch.setattr(g, "send_for_review", lambda paths, cap, key: enviados.update({"caption": cap}))
    monkeypatch.setattr(g, "wait_decision", lambda key, timeout_s=900: ("ok", None))
    monkeypatch.setattr(g, "upload_image", lambda p: "http://img/i.png")
    monkeypatch.setattr(g, "insert_item", lambda it, c, urls: "id1")
    monkeypatch.setattr(g, "set_status", lambda i, s: None)
    g.procesar_item(item)
    assert "caption real de facebook" in enviados["caption"]
